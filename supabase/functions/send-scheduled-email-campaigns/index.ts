...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: dueRows, error: dueErr } = await admin.from("event_email_campaigns").select("id, event_id, egoi_campaign_id, scheduled_send_attempts, segment_id").eq("status", "scheduled").is("scheduled_send_claimed_at", null).lte("scheduled_at", nowIso).order("scheduled_at", { ascending: true }).limit(BATCH_LIMIT);
