...
// supabase/functions/blog-digest-draft/pastEventFilter.ts
function filterOutPastEventPosts(posts, eventLinks, todayIso) {
  const linksByPost = /* @__PURE__ */ new Map();
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: sendOnCronRow } = await admin.from("site_settings").select("value").eq("key", "blog_digest_send_on_cron").maybeSingle();
    ...
    const { data: eventLinksRaw } = await admin.from("events").select("blog_post_id,date,end_date").in("blog_post_id", rawPosts.map((p) => p.id));
    const todayBRT = new Date(now.getTime() - 3 * 60 * 60 * 1e3).toISOString().slice(0, 10);
    const pts = filterOutPastEventPosts(rawPosts, eventLinksRaw ?? [], todayBRT);
    if (pts.length === 0) {
