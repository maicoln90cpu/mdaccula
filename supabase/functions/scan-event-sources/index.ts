...
function buildResendEmailRequest(params) {
  return {
...
// supabase/functions/_shared/autoPublishAlert.ts
function escapeHtml(text) {
  return text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
var SOURCE_LABELS = {
  auto_cron: "Autom\xE1tico (cron)",
  event_watcher: "Event Watcher"
};
async function notifyAutoPublish(supabase, params) {
  try {
    const resendApiKey = Deno.env.get("RESEND_API_KEY");
    if (!resendApiKey) {
      console.warn("[autoPublishAlert] RESEND_API_KEY ausente \u2014 aviso de publica\xE7\xE3o autom\xE1tica n\xE3o enviado.");
      return;
    }
    const { data } = await supabase.from("site_settings").select("value").eq("key", "auto_publish_alert_email").maybeSingle();
    ...
    const html = `
      <div style="font-family: sans-serif; background:#0a0a0a; color:#e5e5e5; padding:24px;">
        <h2 style="color:#a78bfa;">Artigo publicado automaticamente, sem revis\xE3o</h2>
        ...
        <p style="color:#888; font-size:12px;">Pra parar de publicar sozinho, desligue o toggle correspondente na aba "Autom\xE1tico" de Conte\xFAdo IA.</p>
      </div>
    `.trim();
    const request = buildResendEmailRequest({
      resendApiKey,
      toEmail,
      subject: `[MDACCULA] Publicado sem revis\xE3o: ${params.title}`,
      html
    });
    ...
    console.error("[autoPublishAlert] Erro inesperado (n\xE3o afeta a gera\xE7\xE3o):", error);
  }
}
...
async function generateDraftArticle(admin, supabaseUrl, serviceKey, draftId, extracted, templateId, autoPublish) {
  try {
    ...
    if (autoPublish && isContentSubstantial(data.post.content)) {
      ...
      console.warn(`[scan-event-sources] Conte\xFAdo curto demais \u2014 publica\xE7\xE3o autom\xE1tica cancelada, post ${data.post.id} continua rascunho.`);
    }
  } catch (error) {
