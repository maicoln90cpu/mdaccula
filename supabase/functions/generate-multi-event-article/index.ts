...
// supabase/functions/_shared/generateMultiEventArticle/prompts.ts
...
var DEFAULT_SYSTEM_PROMPT = `Voc\xEA \xE9 um jornalista renomado especializado em m\xFAsica eletr\xF4nica brasileira e internacional, escrevendo para um p\xFAblico apaixonado pela cena underground e pelos grandes eventos.
...
var DEFAULT_USER_PROMPT_TEMPLATE = `Escreva um artigo COMPLETO e EXTENSO sobre a s\xE9rie de eventos "{{seriesName}}":
...
function buildDatesInfo(events) {
...
function buildSystemPromptExtras(opts) {
...
function buildOfficialBlock(params) {
  return `\u{1F4CB} DADOS OFICIAIS DA S\xC9RIE (use literalmente, NUNCA invente, NUNCA contradiga):
- S\xE9rie: ${params.seriesName}
- Local comum: ${params.commonVenue}, ${params.commonCity}/${params.commonState}
- Per\xEDodo: ${formatDatePt(params.firstDate)} a ${formatDatePt(params.lastDate)}
- G\xEAneros: ${params.genres.join(", ") || "M\xFAsica Eletr\xF4nica"}
...
${additionalContext}` : "");
    ...
    if (isRegeneration && existingPostId) {
      ...
      if (publishImmediately !== false && !substantial) {
        console.warn(`[generate-multi-event-article] Conte\xFAdo curto demais \u2014 publica\xE7\xE3o autom\xE1tica cancelada, nasce como rascunho: "${articleData.title}"`);
      }
