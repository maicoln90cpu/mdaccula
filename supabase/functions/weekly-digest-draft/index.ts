...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: eventRows }, { data: posts }, { data: tplSettings }, { data: activeTpl }, { data: globalBlocksRows }] = await Promise.all([
      admin.from("events").select("id,title,slug,date,end_date,time,venue,location_city,location_state,image_url,ticket_link,cta_type,status,lineup").eq("status", "active").lte("date", endIso).or(`date.gte.${startIso},end_date.gte.${startIso}`).order("date", { ascending: true }).limit(50),
