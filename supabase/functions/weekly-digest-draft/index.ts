...
function renderBasicBlock(block, ctx, style) {
  ...
  switch (block.kind) {
    ...
    case "event_meta": {
      ...
      const dateLabel = showIcons ? "\u{1F4C5} Data e hora" : "Data e hora";
      const localLabel = showIcons ? "\u{1F4CD} Local" : "Local";
      if (stacked) {
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                ...
                <td style="vertical-align:top;">
                  <div style="color:${primary};font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;margin-bottom:3px;">\u{1F4F0} Leia a mat\xE9ria</div>
    ...
    case "footer": {
      ...
      const unsubscribe = block.include_unsubscribe !== false ? `<p style="margin:8px 0 0 0;font-size:${size}px;"><a href="[E-GOI_UNSUBSCRIBE_LINK]" style="color:#71717a;font-weight:700;text-decoration:underline;">Descadastrar-se</a></p>` : "";
...
function renderDigestBlock(block, ctx, style) {
  ...
  switch (block.kind) {
    case "weekend_grid": {
      ...
      const isDedgeVenue2 = (v) => /d\.?\s*edge/i.test((v || "").trim());
      const list = (event.weekendEvents || []).filter((ev) => ev && (!heroId || ev.id !== heroId) && !isDedgeVenue2(ev.venue));
      const align = block.align ?? "left";
        ...
        return `<td width="50%" style="padding:8px;vertical-align:top;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#0d0d0d;border:1px solid rgba(255,255,255,0.08);border-radius:12px;overflow:hidden;">
            ...
            <tr><td style="padding:12px 14px 14px 14px;">
              <div style="color:${accent};font-size:10px;font-weight:800;letter-spacing:0.15em;text-transform:uppercase;margin-bottom:4px;">${escape(ev.dayLabel)}${ev.timeLabel ? ` \xB7 ${escape(ev.timeLabel)}` : ""}</div>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#0d0d0d;border:1px solid rgba(255,255,255,0.08);border-radius:12px;overflow:hidden;">
            <tr>
              ...
              <td style="padding:12px 14px;vertical-align:top;">
                ...
                ${showReadMore ? `<a href="${url}" style="display:inline-block;margin-top:6px;color:${primary};font-size:10px;font-weight:800;text-decoration:none;text-transform:uppercase;letter-spacing:0.12em;">Ler mat\xE9ria \u2192</a>` : ""}
              </td>
            </tr>
          </table>
        </td></tr>`;
...
// supabase/functions/_shared/weeklyDigestDraft/legacyHtml.ts
...
function renderDigestHtml(events, posts, settings, rangeLabel) {
  ...
  const eventCards = events.length === 0 ? `<tr><td style="padding:12px 20px;color:#bbb;font-family:Arial,sans-serif;font-size:14px;">Nenhum evento confirmado para os pr\xF3ximos 7 dias \u2014 fique de olho no site.</td></tr>` : events.map((e) => {
  ...
  const postCards = posts.length === 0 ? "" : `
    <tr><td style="padding:24px 20px 6px 20px;font-family:Arial,sans-serif;">
      <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:${accent};font-weight:700;">Mat\xE9rias em alta</div>
    </td></tr>` + posts.map((p) => {
  ...
  <tr><td align="center" style="padding:24px 12px;">
    <table role="presentation" cellpadding="0" cellspacing="0" width="600" style="width:600px;max-width:600px;border-collapse:collapse;background:${bg};">
      ...
      <tr><td style="padding:0 20px 8px 20px;font-family:Arial,sans-serif;">
        <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:${accent};font-weight:700;">Resumo semanal \xB7 ${escapeHtml(rangeLabel)}</div>
        <h1 style="font-size:24px;line-height:1.2;color:#fff;margin:6px 0 4px 0;">O que rola na semana</h1>
        <p style="font-size:14px;color:#bbb;margin:0;">Os destaques da agenda e do blog nos pr\xF3ximos dias em S\xE3o Paulo.</p>
      </td></tr>
...
// supabase/functions/_shared/weeklyDigestDraft/buildEventPayload.ts
var SITE_URL2 = "https://mdaccula.com";
var isDedgeVenue = (v) => /d\.?\s*edge/i.test((v || "").trim());
function buildEventPayload(evs, pts, settings, activeTpl, rangeLabel, digestLabel) {
  ...
  const evsForRender = Object.values(groupsMap).map((group) => group.sort((a, b) => a.date.localeCompare(b.date))).flatMap((group) => {
    ...
    const subEvents = group.map((g) => ({
      label: g.title,
      url: g.ticket_link || `${SITE_URL2}/eventos/${g.slug}`,
  ...
  const weekendEvents = nonDedge.map((e) => ({
    ...
    imageUrl: e.image_url || `${SITE_URL2}/placeholder.svg`,
    eventUrl: `${SITE_URL2}/eventos/${e.slug}`,
    ticketUrl: e.ticket_link || `${SITE_URL2}/eventos/${e.slug}`,
  ...
  const dedgePayload = dedgeHead ? {
    ...
    primaryUrl: `${SITE_URL2}/eventos?venue=dedge`,
  ...
  const blogPosts = pts.map((p) => ({
    ...
    url: `${SITE_URL2}/blog/${p.slug}`
  }));
  const settingsAny = settings;
  return {
    eventTitle: first?.title || (digestLabel.includes("FDS") ? "Agenda do fim de semana" : "O que rola na semana"),
    eventSubtitle: `${digestLabel} \xB7 ${rangeLabel}`,
    flyerUrl: first?.image_url || settings.logo_url || `${SITE_URL2}/placeholder.svg`,
    ...
    description: "Os destaques da agenda e do blog nos pr\xF3ximos dias em S\xE3o Paulo.",
    ticketUrl: first ? first.ticket_link || `${SITE_URL2}/eventos/${first.slug}` : `${SITE_URL2}/eventos`,
    eventUrl: first ? `${SITE_URL2}/eventos/${first.slug}` : `${SITE_URL2}/eventos`,
    agendaUrl: `${SITE_URL2}/eventos`,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isCron && cronSecret && cronJobHeader) {
      const { data: row } = await admin.from("internal_cron_secrets").select("secret").eq("name", "weekly_digest_cron").maybeSingle();
    ...
    const range = body?.range === "weekend" ? "weekend" : "week";
    ...
    const { data: digestRow } = await admin.from("site_settings").select("value").eq("key", "weekly_digest_enabled").maybeSingle();
    ...
    const { data: sendOnCronRow } = await admin.from("site_settings").select("value").eq("key", "weekly_digest_send_on_cron").maybeSingle();
    ...
    if (range === "weekend") {
      ...
      rangeStart = now;
      rangeEnd = new Date(now.getTime() + 7 * 24 * 3600 * 1e3);
    }
    const startIso = rangeStart.toISOString().slice(0, 10);
    const endIso = rangeEnd.toISOString().slice(0, 10);
    const templateTypes = range === "weekend" ? ["weekend_agenda"] : ["weekly_digest", "weekly_digest_editorial"];
    ...
    if (overrideTemplateId) {
      ...
      const { data: cfgTplRow } = await admin.from("site_settings").select("value").eq("key", settingKey).maybeSingle();
      const cfgTplId = cfgTplRow?.value && cfgTplRow.value !== "" ? cfgTplRow.value : null;
      if (cfgTplId) {
        activeTplQuery = activeTplQuery.eq("id", cfgTplId);
      } else {
        activeTplQuery = activeTplQuery.in("type", templateTypes).order("is_default", { ascending: false }).order("updated_at", { ascending: false }).limit(1);
      }
    }
    ...
    if (resolvedTplBlocks && resolvedTplBlocks.length > 0) {
      if (!hasAnyBlockKind(
        resolvedTplBlocks,
        range === "weekend" ? ["weekend_grid", "weekly_hero", "dedge_block"] : ["weekend_grid", "weekly_hero", "blog_posts_list", "dedge_block"]
      )) {
        return json({
          ok: false,
          error: range === "weekend" ? "Template de Agenda FDS precisa conter bloco de agenda din\xE2mica." : "Template de Digest semanal precisa conter bloco de agenda ou blog din\xE2mico.",
    ...
    try {
      ...
      console.warn("[weekly-digest-draft] text/preheader gen failed:", e);
    }
