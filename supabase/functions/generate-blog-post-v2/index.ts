...
// supabase/functions/_shared/generateBlogPostV2/egress.ts
...
// supabase/functions/_shared/generateBlogPostV2/http.ts
var corsHeaders = {
...
// supabase/functions/_shared/generateBlogPostV2/contentAnalysis.ts
...
// supabase/functions/_shared/generateBlogPostV2/textUtils.ts
...
// supabase/functions/_shared/generateBlogPostV2/imageGeneration.ts
import { Image } from "https://deno.land/x/imagescript@1.3.0/mod.ts";

// supabase/functions/_shared/generateBlogPostV2/imageStyles.ts
...
// supabase/functions/_shared/generateBlogPostV2/dateHelpers.ts
var WEEKDAYS_PT = [
  "domingo",
  "segunda-feira",
  "ter\xE7a-feira",
...
function computeDateFormatted(dateStr) {
  const m = dateStr?.match?.(/^(\d{4})-(\d{2})-(\d{2})/);
  if (!m) return dateStr || "";
...
// supabase/functions/_shared/generateBlogPostV2/promptBuilder.ts
function applyTemplateVariables(templateStr, formFields) {
...
function buildOfficialDataBlock(formFields, isEventMode) {
...
// supabase/functions/_shared/generateBlogPostV2/savePost.ts
async function generateUniqueSlug(supabase, title) {
...
async function saveOrUpdatePost(supabase, params) {
  const {
    existingPostId,
    publishImmediately,
    eventData,
  ...
  if (existingPostId) {
    ...
    const { data: data2, error: error2 } = await supabase.from("blog_posts").update({
    ...
    return { post: data2, error: error2 };
  }
...
async function logAiGeneration(supabase, params) {
  const promptFieldsSummary = Object.entries(params.formFields).filter(([, value]) => value).map(([key, value]) => `${key}: ${String(value).substring(0, 50)}`).join(" | ");
...
Use estas informa\xE7\xF5es reais para enriquecer o artigo:${scrapedContext}` : "") },
    ...
    if (downgradedForQuality) {
      console.warn(`[generate-blog-post-v2] Conte\xFAdo curto demais \u2014 publica\xE7\xE3o autom\xE1tica cancelada, post ${post?.id} nasceu como rascunho.`);
      try {
