// ============================================
// MDAccula Service Worker v4
// Stale While Revalidate + Cache Strategies
// ============================================

const BUILD_TIMESTAMP = Date.now();
const CACHE_VERSION = 'v5';
const STATIC_CACHE = `mdaccula-static-${CACHE_VERSION}`;
const DYNAMIC_CACHE = `mdaccula-dynamic-${CACHE_VERSION}`;
const IMAGE_CACHE = `mdaccula-images-${CACHE_VERSION}`;
const LINKS_API_CACHE = `mdaccula-links-api-${CACHE_VERSION}`;

// Cache duration configs
const CACHE_DURATIONS = {
  static: 1000 * 60 * 60 * 24 * 30, // 30 dias para fontes/CSS/JS
  images: 1000 * 60 * 60 * 24 * 7,   // 7 dias para imagens
  dynamic: 1000 * 60 * 60 * 24,      // 1 dia para HTML
};

// Assets críticos para cachear no install
const PRECACHE_URLS = [
  '/',
  '/offline.html'
];

// Padrões para recursos estáticos (fonts, CSS, JS bundled)
const STATIC_PATTERNS = [
  /\.woff2?$/,
  /\.ttf$/,
  /\.otf$/,
  /\/assets\/.*\.(js|css)$/,
];

// Padrões para imagens
const IMAGE_PATTERNS = [
  /\.(png|jpg|jpeg|gif|webp|avif|svg|ico)$/
];

// Install event - cache critical resources
self.addEventListener('install', (event) => {
  console.log(`[SW] Installing v${CACHE_VERSION}`);
  
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        console.log('[SW] Caching precache URLs');
        return cache.addAll(PRECACHE_URLS);
      })
      .then(() => {
        console.log('[SW] Installation complete');
      })
      .catch((error) => {
        console.error('[SW] Installation failed:', error);
      })
  );
  
  self.skipWaiting();
});

// Activate event - clean up old caches
self.addEventListener('activate', (event) => {
  console.log(`[SW] Activating v${CACHE_VERSION}`);
  
  const currentCaches = [STATIC_CACHE, DYNAMIC_CACHE, IMAGE_CACHE, LINKS_API_CACHE];
  
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (cacheName.startsWith('mdaccula-') && !currentCaches.includes(cacheName)) {
            console.log('[SW] Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('[SW] Activation complete');
    })
  );
  
  self.clients.claim();
});

// Helper: check if URL matches patterns
const matchesPatterns = (url, patterns) => {
  return patterns.some(pattern => pattern.test(url.pathname));
};

// Helper: check if request should bypass cache
const shouldBypassCache = (url) => {
  const isExternal = url.origin !== self.location.origin;
  const isSupabaseAPI = url.hostname.includes('supabase.co');
  const isAnalytics = url.hostname.includes('googletagmanager.com') || 
                      url.hostname.includes('google-analytics.com') ||
                      url.hostname.includes('hotjar.com') ||
                      url.hostname.includes('facebook.net');
  const isHotReload = url.pathname.includes('/@vite') || 
                      url.pathname.includes('/__vite') ||
                      url.pathname.includes('/node_modules/');
  
  return isExternal || isSupabaseAPI || isAnalytics || isHotReload;
};

// Strategy: Cache First (for static assets like fonts)
const cacheFirst = async (request, cacheName) => {
  const cachedResponse = await caches.match(request);
  if (cachedResponse) {
    return cachedResponse;
  }
  
  try {
    const networkResponse = await fetch(request);
    if (networkResponse && networkResponse.status === 200) {
      const cache = await caches.open(cacheName);
      cache.put(request, networkResponse.clone());
    }
    return networkResponse;
  } catch (error) {
    console.log('[SW] Cache First failed:', request.url);
    throw error;
  }
};

// Strategy: Stale While Revalidate (for images and dynamic content)
const staleWhileRevalidate = async (request, cacheName) => {
  const cache = await caches.open(cacheName);
  const cachedResponse = await cache.match(request);
  
  // Fetch em background para atualizar cache
  const fetchPromise = fetch(request).then((networkResponse) => {
    if (networkResponse && networkResponse.status === 200) {
      cache.put(request, networkResponse.clone());
    }
    return networkResponse;
  }).catch((error) => {
    console.log('[SW] SWR fetch failed, using cache:', request.url);
    return cachedResponse;
  });
  
  // Retorna cache imediatamente se disponível, senão espera network
  return cachedResponse || fetchPromise;
};

// Strategy: Network First with Cache Fallback (for HTML pages)
const networkFirst = async (request, cacheName) => {
  try {
    const networkResponse = await fetch(request);
    if (networkResponse && networkResponse.status === 200) {
      const cache = await caches.open(cacheName);
      cache.put(request, networkResponse.clone());
    }
    return networkResponse;
  } catch (error) {
    console.log('[SW] Network First failed, trying cache:', request.url);
    const cachedResponse = await caches.match(request);
    if (cachedResponse) {
      return cachedResponse;
    }
    
    // Fallback para página offline em navegações
    if (request.mode === 'navigate') {
      return caches.match('/offline.html');
    }
    
    throw error;
  }
};

// Fetch event handler
self.addEventListener('fetch', (event) => {
  if (event.request.method !== 'GET') {
    return;
  }

  const url = new URL(event.request.url);

  if (shouldBypassCache(url)) {
    return;
  }

  // Static assets (fonts, bundled JS/CSS) - Cache First
  if (matchesPatterns(url, STATIC_PATTERNS)) {
    event.respondWith(cacheFirst(event.request, STATIC_CACHE));
    return;
  }

  // Images - Stale While Revalidate (fast + fresh)
  if (matchesPatterns(url, IMAGE_PATTERNS)) {
    event.respondWith(staleWhileRevalidate(event.request, IMAGE_CACHE));
    return;
  }

  // Supabase API calls for links - Stale While Revalidate for fast subsequent loads
  if (url.pathname.includes('/rest/v1/link_groups') || url.pathname.includes('/rest/v1/site_settings')) {
    event.respondWith(staleWhileRevalidate(event.request, LINKS_API_CACHE));
    return;
  }

  // HTML and other dynamic content - Network First
  event.respondWith(networkFirst(event.request, DYNAMIC_CACHE));
});

// Message event - permitir comunicação com o app
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'SKIP_WAITING') {
    console.log('[SW] Skip waiting requested');
    self.skipWaiting();
  }
  
  if (event.data && event.data.type === 'GET_VERSION') {
    event.ports[0].postMessage({
      version: CACHE_VERSION,
      buildTimestamp: BUILD_TIMESTAMP,
      caches: {
        static: STATIC_CACHE,
        dynamic: DYNAMIC_CACHE,
        images: IMAGE_CACHE
      }
    });
  }
  
  if (event.data && event.data.type === 'CLEAR_CACHE') {
    event.waitUntil(
      caches.keys().then((names) => 
        Promise.all(names.map((name) => caches.delete(name)))
      ).then(() => {
        event.ports[0].postMessage({ success: true });
      })
    );
  }
});

// Periodic sync para limpeza de cache (se suportado)
self.addEventListener('periodicsync', (event) => {
  if (event.tag === 'cache-cleanup') {
    event.waitUntil(cleanupExpiredCaches());
  }
});

// Cleanup helper
async function cleanupExpiredCaches() {
  const currentCaches = [STATIC_CACHE, DYNAMIC_CACHE, IMAGE_CACHE, LINKS_API_CACHE];
  const cacheNames = await caches.keys();
  const oldCaches = cacheNames.filter(name => 
    name.startsWith('mdaccula-') && !currentCaches.includes(name)
  );
  
  await Promise.all(oldCaches.map(name => caches.delete(name)));
  console.log('[SW] Cleanup complete, removed:', oldCaches.length, 'old caches');
}
