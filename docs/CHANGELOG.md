...
- **O que:** investigando o achado incidental do item de pendência anterior (chamadas `401` repetidas em `cleanup-storage`), descobri que não era ruído de scanner externo — era o próprio cron semanal do projeto (`cleanup-storage-weekly`, agendado desde 10/05/2026 em `20260510130630_...sql`) falhando de verdade toda semana. O job manda `Authorization: Bearer <anon key>`, mas a função exige `authorizeAdminOrCron` (header `x-cron-secret`/`x-cron-job` batendo com uma linha em `internal_cron_secrets`, ou sessão de admin real) — a anon key não satisfaz nenhum dos dois. `cleanup-sync-logs-weekly` (mesmo arquivo, mesmo padrão) tinha exatamente o mesmo problema.
- **Por que passou despercebido:** o `pg_cron` sempre marca essas execuções como `"succeeded"` (`cron.job_run_details`), porque `net.http_post` só confirma que a chamada HTTP foi enfileirada de forma assíncrona — nunca checa o status da resposta. Confirmado diretamente: a chamada de `cleanup-storage` de 16/08 às 04:00:10 UTC (exatamente a hora do cron) recebeu `401` real, junto com todas as execuções anteriores desde maio — ou seja, a limpeza automática de Storage e de `sync_logs` nunca rodou de verdade.
- **Correção:** cadastrados 2 segredos novos em `internal_cron_secrets` (`cleanup_storage_cron`, `cleanup_sync_logs_cron`) e os 2 jobs reagendados pra mandar `x-cron-job`/`x-cron-secret` em vez da anon key — mesmo padrão já usado nos outros crons internos (ex.: `heal-stuck-email-dispatches`). Testado manualmente via `net.http_post` direto (sem esperar o próximo domingo): as 2 funções agora respondem `200 OK` — `cleanup-storage` seguiu em modo simulação (comportamento já era esse antes, não mudou: só reporta o que apagaria, 98 arquivos órfãos / 6.81 MB encontrados no teste), `cleanup-sync-logs` de fato roda a exclusão de logs com mais de 30 dias (comportamento pretendido desde o início, nunca tinha rodado).
- **Achado à parte:** o relatório de simulação do `cleanup-storage` listou `"blog-images"` como arquivo órfão — falso positivo, corrigido na entrada seguinte deste changelog.
- **Reclassificado:** o item anterior sobre "scanner batendo em todas as functions" foi reauditado na entrada seguinte — nenhuma função (incluindo essas 2) está vazando dado, todas exigem auth corretamente.
- **Arquivos:** `supabase/migrations/20260820225000_fix_cleanup_cron_jobs_auth.sql`.

### 20/08/2026 — Prevenção de regressão (R-084) pro incidente dos crons quebrados + corrige falso positivo no `cleanup-storage` + audita scanner externo

- **Prevenção de regressão:** o incidente anterior (crons falhando com 401 por meses, sem alerta, porque `pg_cron` marca `net.http_post` como "succeeded" independente da resposta HTTP real) ganhou um mecanismo de detecção. `authorizeAdminOrCron` (`supabase/functions/_shared/index.ts`) agora grava um heartbeat em `cron_job_health.last_success_at` toda vez que um cron passa no auth de verdade. Nova function `cron-health-check`, agendada 1x/dia (13h UTC), compara esse carimbo com um limite de atraso esperado por job (`expected_max_gap_hours`) e manda e-mail de alerta (+ grava em `cron_health_alerts`) se algum cron parar de bater o heartbeat — mesmo padrão de e-mail do `egress-alert-cron`. Cobre os 7 crons ativos hoje que usam `authorizeAdminOrCron`: `auto-article-cron`, `create-recurring-events`, `cleanup-storage`, `cleanup-sync-logs`, `daily-metrics-email`, `scan-event-sources`, `verify-sources-weekly`. Não cobre crons com auth inline própria (ex.: `egress-alert-cron`, `heal-stuck-email-dispatches`) — ficam de fora por ora.
- **Falso positivo do `cleanup-storage` corrigido:** a pasta `"blog-images"` aparecia como "arquivo órfão" no relatório de simulação porque o `storage.list()` do Supabase devolve subpastas como itens com `id: null` (placeholder, não é arquivo) — a comparação com `referencedUrls` não tinha como saber disso. Filtrado antes do cálculo de órfãos/duplicados.
- **Scanner externo reauditado:** grep em todas as 57 Edge Functions procurando qualquer forma de auth — 54 têm checagem inline, as 3 sem nenhuma (`public-maps-config`, `indexnow-notify`, `render-static-map`) são intencionalmente públicas, com justificativa documentada no próprio código, e nenhuma expõe dado sensível. Confirmado: nenhuma vulnerabilidade encontrada; item de monitoramento em `docs/PENDENCIAS.md` atualizado com o resultado.
- **Arquivos:** `supabase/migrations/20260820233000_cron_job_health_check.sql`, `supabase/functions/cron-health-check/index.ts` (novo), `supabase/functions/_shared/index.ts`, `supabase/functions/cleanup-storage/index.ts`, `supabase/config.toml`.

### 20/08/2026 — Corrigido histórico de migrations desatualizado, causa raiz provável do branch `MIGRATIONS_FAILED`

- **O que:** investigado o checkpoint aberto sobre o branch de produção do Supabase que já apareceu com `status: MIGRATIONS_FAILED`. O branch em si já não existe mais em `list_branches`, mas a investigação encontrou a causa raiz provável: a tabela de controle `supabase_migrations.schema_migrations` só tinha registro de migrations a partir de 19/02/2026 (100 entradas), enquanto o repositório tem 66 arquivos de migration mais antigos (14/09/2025 a 18/02/2026) já aplicados no schema real (tabelas como `profiles` e `events` já existiam), mas nunca registrados no histórico de controle.
- **Por que isso derruba a criação de branch:** ao criar um branch de preview, o Supabase tenta reaplicar a sequência de migrations dos arquivos do repositório contra um banco novo; um histórico de controle incompleto na produção é o padrão clássico que causa esse tipo de falha.
- **Correção:** backfill dos 66 registros faltantes em `supabase_migrations.schema_migrations` (`version` + `name`, sem reexecutar SQL nenhum — só marcando como já aplicado, que já era verdade). Total de migrations rastreadas foi de 100 para 166.
- **Achado à parte:** a checagem também revelou dezenas de chamadas `POST 401` à função `cleanup-storage` — investigado e corrigido na mesma sessão, ver entrada seguinte neste changelog.
- **Prevenção:** nova nota de hábito documentada — ver `docs/tabelas.md`.
...
- **Correção:** nova propriedade `link_target` no bloco `weekend_grid` (mesmos valores `ticket_link`/`event_url`, mesmo controle no editor), aplicada nos 3 layouts (cartaz, timeline e grid — inclusive o card único de 1 evento) e na versão texto-puro. Reaproveita o mesmo `resolveGridCardUrl`.
- **Nota sobre comportamento padrão:** como o botão "Garantir ingresso" desses blocos sempre foi direto pra ticketeira, o padrão (`ticket_link`) agora unifica também a imagem/nome pro mesmo destino — antes, imagem/nome iam sempre pra página do site e só o botão ia pra ticketeira (dois destinos diferentes no mesmo card, sem opção de escolher). Quem quiser o comportamento "imagem/nome no site" de volta escolhe `event_url` (que agora também unifica o botão pra lá). Essa mudança de padrão já valia desde a entrada anterior pro `event_grid`; agora vale igual pro `weekend_grid`.
- **Validação:** `tsc`, `lint` e os 734 testes verdes (5 novos cobrindo os 3 layouts); suíte Deno (317 testes) também verde.
- **Arquivos:** `supabase/functions/_shared/emailBlocks/types.ts`, `supabase/functions/_shared/emailBlocks/renderBlock/digest.ts`, `supabase/functions/_shared/emailBlocks/renderBlockedTemplateText.ts`, `src/components/admin/emailTemplateEditor/blockPropsPanel/digestProps.tsx`, `src/components/admin/emailTemplateEditor/blockDefaults.ts`, `src/__tests__/lib/blocks-grid-cards.test.ts`.

### 20/08/2026 — Grid de eventos (virada de lote) ganha opção de link: ticketeira externa ou página do evento no site

- **O que:** o bloco `event_grid` (grade de múltiplos eventos usada na "virada de lote") sempre mandava o clique no card inteiro (imagem, nome e botão "Garantir ingresso") direto pra ticketeira externa (Sympla, Ingresse, etc). Agora dá pra escolher, por bloco, entre esse comportamento (padrão) ou mandar tudo pra página do próprio evento dentro do site mdaccula.
- **Onde mexer:** editor de e-mail → bloco "Grid de evento..." → propriedade nova "Link dos cards (imagem, nome e botão)".
- **Correção:** nova propriedade `link_target` (`ticket_link` | `event_url`, padrão `ticket_link`) no bloco `event_grid`; um resolvedor único (`resolveGridCardUrl`, `supabase/functions/_shared/emailBlocks/utils.ts`) decide a URL usada tanto pela versão HTML (imagem, nome e botão do card, `renderGridEventCard` em `renderBlock/digest.ts`) quanto pela versão texto-puro (`renderBlockedTemplateText.ts`) do e-mail — sempre a mesma escolha nos dois.
- **Nota:** antes desta entrada, imagem/nome do card sempre iam pra página do evento no site e só o botão ia pra ticketeira — o padrão novo (`ticket_link`) unifica os dois no mesmo destino que o botão já usava, então templates já salvos passam a levar o clique na imagem/nome pra ticketeira também (mudança pequena de comportamento, não só uma opção nova — ver a entrada acima, que estendeu o mesmo ajuste pro `weekend_grid`).
...
**Proteção:** `src/__tests__/regression/brand-icons-survive-lucide-v1.test.tsx` (R-077).
...
**Impacto:** fecha a última lacuna conhecida da família de bugs "evento trava reservado sem rastro" (R-077/057/058/059/062/064) — agora TODA etapa entre o claim e a confirmação final (rede E rede+banco) tem timeout.
...
### Fix: evento ficava travado ("dispatch_in_progress") sem log de erro em falhas não previstas (R-077)
...
| Data     | Tipo             | Descrição                                                                                                                                                            |
| -------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 20/08    | Infra/Segurança  | Prevenção de regressão (heartbeat + alerta de cron parado), fix de falso positivo no `cleanup-storage`, auditoria de auth em todas as 57 Edge Functions              |
| 20/08    | Bugfix/Infra     | Crons `cleanup-storage-weekly`/`cleanup-sync-logs-weekly` falhavam com 401 toda semana desde maio, silenciosamente — corrigida a autenticação                       |
| 20/08    | Infra            | Backfill de 66 migrations sem registro no histórico de controle do Supabase — causa raiz provável do branch `MIGRATIONS_FAILED`                                     |
...
| 10/08    | Bugfix           | Evento ficava travado ("dispatch_in_progress") sem log em falhas não previstas + fetches de mapa sem timeout (R-077)                                                 |
...
_Registre uma entrada nova aqui sempre que uma implementação relevante for concluída — não deixe pendurado em `PENDENCIAS.md`._
