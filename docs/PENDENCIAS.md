...
**Status:** 🔧 Fases 1-5 de 8 concluídas (16/08/2026) — `send-mass-newsletter`, `import-csv-data`, `upload-csv` (Fase 1), `cleanup-storage` (Fase 2), `auto-article-cron`, `create-recurring-events`, `cleanup-sync-logs`, `verify-sources-weekly` (Fase 3), `generate-blog-post-v2`, `generate-blog-post-from-topic`, `generate-blog-suggestions` (Fase 4), `generate-multi-event-article`, `regenerate-blog-image`, `preview-topic-sources` (Fase 5) já exigem admin autenticado (ou cron-secret/service-role-key própria, conforme o caso) em produção — confirmado por contract tests rodando contra a function real já deployada, respeitando a lição da Fase 4 (só rodar depois do deploy). Fases 6-8 (6 functions restantes) ainda abertas.
**Contexto:** `verify_jwt` é `false` em todas as functions do projeto (auth é responsabilidade de cada function). Funções ainda sem checagem, pensadas como "só admin usa": `diagnose-media`, `batch-convert-webp`, `convert-to-webp`, `import-storage`, `fetch-link-metadata`, `systemhealth`. O padrão usado nas fases anteriores (`authorizeAdminOrCron()` em `supabase/functions/_shared/index.ts`) é reaproveitável nas próximas — inclusive a opção nova `allowServiceRole` (Fase 4), pra functions chamadas server-to-server por outras Edge Functions usando a service role key.
**⚠️ Lição da Fase 4 (16/08/2026, aplicada com sucesso na Fase 5):** contract tests de auth de functions que geram conteúdo por IA/gastam recurso externo só devem rodar DEPOIS do deploy da correção — rodar antes, contra a function ainda sem guarda, dispara trabalho real. Aconteceu de verdade na Fase 4: 6 posts de teste ("teste anuncia...") chegaram a ser publicados de verdade no blog antes de serem identificados e apagados manualmente (`ai_generated_posts` + `blog_posts`). Sem dado vazado, sem custo relevante, mas ficou registrado no código dos testes novos como aviso — e a Fase 5 já seguiu essa regra sem incidente.
