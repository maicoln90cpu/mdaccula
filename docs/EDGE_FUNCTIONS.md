...
**Os 2 casos à parte (16/08/2026) já foram corrigidos**, cada um com o tratamento certo em vez de cópia mecânica do padrão: `send-podcast-notification` ganhou rate limiting por IP (segue pública de propósito — formulário real, `Podcast.tsx`); `compose-event-image` passou a exigir admin-JWT ou service-role (3 chamadores reais: `scan-event-sources`/`apify-instagram-webhook` server-to-server, e o botão de teste manual em `/admin/settings`). Ver `docs/CHANGELOG.md`.
...
| send-podcast-notification | Envia e-mails de confirmação (artista) e notificação (agência) na submissão do podcast | Frontend (formulário público) | Público (rate limit por IP) | Próprio (inline) |
...
| compose-event-image | Aplica marca MDAccula (barra + logo) sobre uma imagem de evento e re-hospeda no Bunny | Interno (scan-event-sources/apify-instagram-webhook) e Frontend (admin) | Admin ou service role (chamada interna) | Próprio (inline) |
