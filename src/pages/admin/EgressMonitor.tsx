            ...
            <Tabs defaultValue="bunny" className="w-full">
                ...
                {bunny?.chunks && bunny.chunks.errors > 0 ? (
                  <Card className="border-yellow-500/40 bg-yellow-500/5">
                    <CardContent className="p-3 text-xs text-muted-foreground flex items-center gap-2">
                      ...
                      Dados parciais: {bunny.chunks.ok} janelas válidas, {bunny.chunks.errors}{' '}
                      ignoradas ({bunny.chunks.stopReason || '—'}). Período real coberto:{' '}
                      {bunny.window.days}d.
                    </CardContent>
                  </Card>
                ) : null}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                  ...
                  <Card variant={(bunny?.pullZone.errors.err5xx || 0) > 100 ? 'warning' : 'metric'}>
                    <CardHeader className="p-4 pb-2">
                      ...
                      <CardTitle className="text-2xl">
                        {(bunny?.pullZone.errors.err4xx || 0) +
                    ...
                    <CardContent className="p-4 pt-0">
                      <ChartContainer config={chartConfig} className="h-[260px] w-full">
                        ...
                        >
                          ...
                          <ChartTooltip
                            content={
                              <ChartTooltipContent formatter={(v) => formatBytes(v as number)} />
                            }
                          />
                          <Area
                            type="monotone"
                        ...
                        >
                          <BarChart
                            data={geoTop.map((g) => ({
                              ...g,
                              label:
                                g.country.length > 22 ? g.country.slice(0, 20) + '…' : g.country,
                            }))}
                            layout="vertical"
              ...
              <TabsContent value="supabase" className="space-y-6">
                ...
                <Card className="border-border/50 bg-muted/30">
                  <CardContent className="p-4 text-sm text-muted-foreground">
                    <strong className="text-foreground">Plano Free:</strong> Supabase só expõe
                    contagem de requisições (não bytes) e janela máxima de 7 dias via Management
                    API. O endpoint Prometheus/metrics requer plano Pro. Para egress real consulte
                    sempre a aba Bunny CDN.
                  </CardContent>
                </Card>
                  ...
                  <Card variant="metric">
                    <CardHeader className="p-4 pb-2">
                      ...
                      <CardTitle className="text-2xl">
                        {sbLoading
                          ? '...'
                          : formatNumber(sbData?.edgeFunctions?.totalInvocations || 0)}
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="p-4 pt-0">
                      <p className="text-xs text-muted-foreground">
                  ...
                  <Card>
                    <CardHeader className="p-4 pb-2">
                      <CardTitle className="text-base">
                        Requests por Serviço (série horária)
                      </CardTitle>
                      <CardDescription>
              ...
              <TabsContent value="history" className="space-y-6">
                <Card className="border-border/50 bg-muted/30">
                  <CardContent className="p-4 text-sm text-muted-foreground flex items-center justify-between gap-3 flex-wrap">
                    <div>
                      <strong className="text-foreground">Snapshots diários</strong> capturados às
                      06:00 UTC (03:00 BRT) via cron. Use o botão para forçar uma captura agora
                      (popula o gráfico antes do primeiro cron rodar).
                    </div>
                    <div className="flex items-center gap-2">
                      {captureMsg && <span className="text-xs">{captureMsg}</span>}
                      <Button size="sm" onClick={captureNow} disabled={capturing}>
                        <RefreshCw
                          className={`h-4 w-4 mr-1.5 ${capturing ? 'animate-spin' : ''}`}
                        />
                        Capturar agora
                      </Button>
                    </div>
                ...
                <Card>
                  <CardHeader className="p-4 pb-2">
                    <CardTitle className="text-base">
                      Bunny Bandwidth (lifetime acumulado)
                    </CardTitle>
                    <CardDescription>
                      Bytes totais entregues pela CDN ao longo do tempo
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    {snapshots.length > 0 ? (
                      ...
                      {snapshots.length > 0 ? (
                        <ChartContainer config={chartConfig} className="h-[260px] w-full">
                          ...
                          >
                            ...
                            <ChartTooltip
                              content={
                                <ChartTooltipContent formatter={(v) => formatNumber(v as number)} />
                              }
                            />
                            <Line
                              type="monotone"
              ...
              <TabsContent value="internal" className="space-y-6">
                <Card className="border-border/50 bg-muted/30">
                  <CardContent className="p-4 text-sm text-muted-foreground flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p>
                        Cobertura ~80%. Não inclui primeira visita, dashboard, bots e preview
                        Lovable. Use como tendência interna; valores oficiais estão nas outras abas.
                      </p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <Card
                    variant={usagePercent > 80 ? 'warning' : usagePercent > 50 ? 'note' : 'success'}
                  >
                    <CardHeader className="p-4 pb-2">
