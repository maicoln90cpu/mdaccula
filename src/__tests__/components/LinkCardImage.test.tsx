...
const SUPABASE_ORIGIN =
  "https://xfvpuzlspvvsmmunznxw.supabase.co/storage/v1/object/public";
...
describe("LinkCardImage", () => {
  ...
  it("pede a variante thumb (não a full) quando otimizado", () => {
    render(
      <LinkCardImage thumbnailUrl={`${BUNNY}/link-thumbnails/foo.webp`} alt="Link" />
    );
  ...
  it("cadeia de fallback: thumb -> full -> Supabase -> fallbackUrl", () => {
    ...
    render(
      <LinkCardImage thumbnailUrl={thumbUrl} fallbackUrl={fallbackUrl} alt="Link" />
    );
    const img = screen.getByRole("img") as HTMLImageElement;
