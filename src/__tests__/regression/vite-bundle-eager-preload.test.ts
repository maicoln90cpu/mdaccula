...
describe("Regressão R-010 — manualChunks não deve forçar icons/charts eager em toda página", () => {
  it("vite.config.ts não agrupa lucide-react/recharts como chunk manual único", () => {
    ...
    expect(
      manualChunksMatch,
      "Não encontrei o bloco manualChunks em vite.config.ts."
    ).toBeTruthy();
    ...
    expect(block, "'lucide-react' não deveria aparecer dentro de manualChunks.").not.toMatch(/lucide-react/);
    expect(block, "'recharts' não deveria aparecer dentro de manualChunks.").not.toMatch(/recharts/);
  });
});
