/**
 * Contract test — Edge Function `batch-convert-webp` (auth).
 *
 * Baixa/reenvia arquivos reais pro Bunny CDN e atualiza URLs no banco via
 ...
 * Fase 6 de 8). Verifica só o CONTRATO de autenticação:
...
const ANON_KEY =
  ...
  '';
const FN_URL = SUPABASE_URL ? `${SUPABASE_URL}/functions/v1/batch-convert-webp` : '';

describe.skipIf(!SUPABASE_URL)('Contract: batch-convert-webp (auth)', () => {
  ...
  it('Sem auth → 401 com JSON de erro (nunca move/converte arquivos)', async () => {
