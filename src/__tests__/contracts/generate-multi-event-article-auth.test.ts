/**
 * Contract test — Edge Function `generate-multi-event-article` (auth).
 *
 * Gera artigo de blog reunindo vários eventos via IA (custo real) e publica
 * no banco via service_role — antes desta correção não tinha NENHUMA
 * checagem de auth no código (achado na auditoria de 15-16/08/2026, ver
 * docs/PENDENCIAS.md, Fase 5 de 8). Verifica só o CONTRATO de autenticação:
...
const ANON_KEY =
  ...
  '';
const FN_URL = SUPABASE_URL ? `${SUPABASE_URL}/functions/v1/generate-multi-event-article` : '';

describe.skipIf(!SUPABASE_URL)('Contract: generate-multi-event-article (auth)', () => {
  ...
  it('Sem auth → 401 com JSON de erro (nunca dispara geração)', async () => {
    const res = await fetch(FN_URL, {
      ...
      body: JSON.stringify({ eventIds: ['00000000-0000-0000-0000-000000000000', '00000000-0000-0000-0000-000000000001'], seriesName: 'teste' }),
    });
