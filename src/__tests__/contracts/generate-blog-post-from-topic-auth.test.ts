/**
 * Contract test — Edge Function `generate-blog-post-from-topic` (auth).
 *
 * Gera artigo de blog via IA (custo real, inclui Firecrawl) e publica no
 * banco via service_role — antes desta correção não tinha NENHUMA checagem
 * de auth no código (achado na auditoria de 15-16/08/2026, ver
 * docs/PENDENCIAS.md, Fase 4 de 8). Verifica só o CONTRATO de autenticação
 * (o comportamento de geração em si já é coberto por outros contract/
 * regression tests deste diretório):
 ...
 *
 * NÃO testa o caminho de chamada interna (auto-article-cron usando a
 * SUPABASE_SERVICE_ROLE_KEY como Bearer, via authorizeAdminOrCron(...,
 * { allowServiceRole: true })) — a service key nunca deve aparecer num
 * arquivo versionado, nem de teste.
 *
...
describe.skipIf(!SUPABASE_URL)('Contract: generate-blog-post-from-topic (auth)', () => {
