...
function makeFunctionsHttpError(
  ...
  (error as Error & { context: Response }).context = new Response(JSON.stringify(body), {
    status: 404,
  });
  return error;
}
...
describe('getEdgeFunctionErrorMessage', () => {
  it('extrai a mensagem real do corpo JSON de um FunctionsHttpError', async () => {
    const error = makeFunctionsHttpError({
      error: 'Nenhuma fonte real encontrada para "a liga".',
      success: false,
    });
    await expect(getEdgeFunctionErrorMessage(error)).resolves.toBe(
      'Nenhuma fonte real encontrada para "a liga".'
    );
  });
  ...
  it('cai pro error.message quando não há context (Error comum)', async () => {
    await expect(getEdgeFunctionErrorMessage(new Error('falha de rede'))).resolves.toBe(
      'falha de rede'
    );
  });
