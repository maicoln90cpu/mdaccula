...
const convertToWebPMock = vi.fn(
  async (_file: unknown, _maxSizeMB: number, maxDimension: number) =>
...
const uploadImageToBunnyMock = vi.fn(
  async () => 'https://mdaccula.b-cdn.net/event-images/abc123-thumb.webp'
);
vi.mock('@/lib/bunnyUploader', () => ({
  uploadImageToBunny: (...args: Parameters<typeof uploadImageToBunnyMock>) =>
    uploadImageToBunnyMock(...args),
}));
...
function mockFromForTables(
  eventsData: { image_url: string | null }[],
  configsData: { image_url: string | null }[]
) {
  ...
  it('propaga erro do Supabase', async () => {
    fromMock.mockImplementation((table: string) => {
      if (table === 'events') {
        return {
          select: () => ({
            not: () => ({ or: () => Promise.resolve({ data: null, error: new Error('boom') }) }),
          }),
        };
      }
...
describe('backfillVariantsForUrl', () => {
  ...
  it('quando nenhuma variante existe, baixa a original e sobe thumb+medium', async () => {
    ...
    const variants = uploadImageToBunnyMock.mock.calls
      .map((c) => (c[2] as { variant: string }).variant)
      .sort();
    expect(variants).toEqual(['medium', 'thumb']);
    // baseName consistente entre as duas variantes, extraído da URL original
    const baseNames = uploadImageToBunnyMock.mock.calls.map(
      (c) => (c[2] as { baseName: string }).baseName
    );
