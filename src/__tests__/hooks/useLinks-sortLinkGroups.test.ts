...
describe('sortLinkGroups — ordem de grupos em /links', () => {
  it('coloca Redes Sociais e Navegação no topo', () => {
    const out = sortLinkGroups([g('JULHO/26', 202607), g('Navegação', 4), g('Redes Sociais', 0)]);
  ...
  it('aceita nome completo (JANEIRO/26) e abreviado (JAN/26)', () => {
    const out = sortLinkGroups([g('FEV/26', 100), g('JANEIRO/26', 200)]);
  ...
  it('não confunde REVEILLON 25/26 com um grupo de mês', () => {
    ...
    expect(out.map((x) => x.name)).toEqual(['Redes Sociais', 'JULHO/26', 'REVEILLON 25/26']);
  });
});
