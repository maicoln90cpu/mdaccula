/**
 * Contrato único de assunto + preheader para templates de e-mail.
 *
 * Usado por preview, envio de teste e criação/envio de campanha para evitar
 * divergência entre o que o admin vê e o que chega no cliente de e-mail.
 */
...
export function resolveEmailPlaceholders(
  ...
  return template.replace(PLACEHOLDER_RE, (match, key: string) => values[key] ?? match).trim();
}
