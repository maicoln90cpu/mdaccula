/**
 * Presets de template — builders de blocos.
 *
 * Extraído de `blocks.ts` na Onda 26 para manter o arquivo principal
 * abaixo de 600 linhas. Consumido via reexport do barrel `blocks.ts`.
 */
import type { Block, SocialNetwork } from '@shared/emailBlocks.ts';
import { newBlockId } from './blockIds';
  ...
  if (type === 'ticket_batch') {
    return [
      ...
      },
      {
        id: newBlockId(),
        kind: 'cta_button',
        label: 'Garantir ingresso agora',
        url_field: 'ticket_link',
      },
      { id: newBlockId(), kind: 'pix_button' },
