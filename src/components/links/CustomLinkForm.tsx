...
export const CustomLinkForm = ({
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) {
        toast({
          variant: 'destructive',
          title: 'Erro',
          description: 'Imagem muito grande (máx 2MB)',
        });
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const finalColorGradient =
        colorGradient === 'default' || colorGradient === '' ? null : colorGradient;
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            disabled={loading}
          />
          <p className="text-xs text-muted-foreground">Sobrescreve a data do evento vinculado</p>
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            disabled={loading}
          />
          <p className="text-xs text-muted-foreground">Sobrescreve o horário do evento vinculado</p>
        </div>
      </div>
            ...
            <SelectValue placeholder="Usar Padrão do Template">
              {colorGradient && colorGradient !== '' ? (
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {GRADIENT_OPTIONS.find((o) => o.value === colorGradient)?.label ||
                      'Personalizado'}
                  </span>
                </div>
              ) : (
      ...
      <div className="mt-4 p-4 border rounded-md bg-muted/30">
        ...
        >
          ...
          <div className="flex-1 min-w-0 text-left">
            <h3 className="font-semibold text-sm truncate">{title || 'Título do Link'}</h3>
            {subtitle && <p className="text-xs opacity-90 truncate mt-1">{subtitle}</p>}
          </div>
        </div>
