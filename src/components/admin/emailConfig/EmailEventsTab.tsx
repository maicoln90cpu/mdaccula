                      ...
                      <TableRow>
                        <TableCell className="w-8">
                          <button
                            type="button"
                            onClick={() =>
                              setExpanded((prev) => ({ ...prev, [entry.event.id]: !open }))
                            }
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Isso registra que <strong>{entry.event.title}</strong> teve o
                                    e-mail disparado manualmente pela E-goi. Você pode desfazer
                                    depois.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => markManual(entry)}>
                                    Confirmar
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                              ...
                              {entry.campaigns.length === 0 ? (
                                ...
                                <div className="divide-y">
                                  {entry.campaigns.map((c) => {
                                    ...
                                    const canShowStats =
                                      c.status === 'sent' && !!c.egoi_campaign_id;
                                    return (
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap">
                                              ...
                                              {c.campaign_type === 'ab_subject' &&
                                                (() => {
                                                  ...
                                                  const metricKey =
                                                    cfg.winner_metric === 'clicks'
                                                      ? 'click_rate'
                                                      : 'open_rate';
                                                  const partner = entry.campaigns.find(
                                                    (x) =>
                                                      x.id !== c.id &&
                                                      x.ab_group_id === c.ab_group_id
                                                  );
                                                  const myStats = campaignStats[c.id];
                                                  const partnerStats = partner
                                                    ? campaignStats[partner.id]
                                                    : null;
                                                  let winnerLabel: string | null = null;
                                                  if (myStats && partnerStats) {
                                                    const mine =
                                                      (myStats[metricKey] as number) ?? 0;
                                                    const theirs =
                                                      (partnerStats[metricKey] as number) ?? 0;
                                          ...
                                          {canShowStats && (
                                            <Button
                                              ...
                                              variant="ghost"
                                              disabled={
                                                !masterEnabled || refreshingStatsId === c.id
                                              }
                                              onClick={() => refreshCampaignStats(c.id)}
                                              title={
                                                masterEnabled
                                                  ? 'Puxar métricas da E-goi'
                                                  : 'Master switch desligado'
                                              }
                                            >
                                              <RefreshCw
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        ...
                                        <b>{entry.event.title}</b>. Na próxima ação de envio, uma{' '}
                                        <b>nova</b> campanha será criada (o histórico anterior é
                                    ...
                                    <AlertDialogFooter>
                                      <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                      <AlertDialogAction
                                        onClick={() => resendEvent(entry.event.id)}
                                      >
                                        Sim, liberar reenvio
                                      </AlertDialogAction>
                                    </AlertDialogFooter>
