...
export function useEmailAutomation({ templates, toast }: UseEmailAutomationInput) {
  ...
  const weeklyEffectiveTemplateId = useMemo(
    () =>
      ...
      templates.find(
        (t) => (t.type === 'weekly_digest' || t.type === 'weekly_digest_editorial') && t.is_default
      )?.id ||
      templates.find((t) => t.type === 'weekly_digest' || t.type === 'weekly_digest_editorial')
        ?.id ||
      '',
  ...
  const handleSaveWeekly = async () => {
    ...
    try {
      await saveAutomation('weekly_digest', {
        ...weeklyCfg,
  ...
  const handleSaveWeekend = async () => {
    ...
    try {
      await saveAutomation('weekend_agenda', {
        ...weekendCfg,
  ...
  const sendAutomationTest = async (
    ...
    try {
      ...
      const { data: sent, error: sendErr } = await supabase.functions.invoke('send-test-email', {
        body: {
          html: res.html,
          subject: `[TESTE] ${res.subject}`,
          to_email: AUTOMATION_TEST_RECIPIENT,
        },
      });
