  ...
  return (
    <div className="space-y-4 sm:space-y-6">
              ...
              {Object.entries(migrateResult.results || {}).map(([bucket, info]) => (
                <div key={bucket} className="space-y-1">
                  <p className="font-medium">
                    {bucket}: {info.migrated} migrados, {info.skipped} existentes, {info.total}{' '}
                    total
                  </p>
                  {info.hasMore && (
      ...
      <Card className="border-blue-500/20">
        ...
        <CardContent className="space-y-4 px-4 sm:px-6">
          <Button
            onClick={handleBackfillEventImages}
          ...
          >
            {backfillRunning ? (
              <>
                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                Processando
                {backfillProgress ? ` (${backfillProgress.done}/${backfillProgress.total})` : '...'}
              </>
            ) : (
              <>
      ...
      <Card className="border-purple-500/20">
        ...
        <CardContent className="space-y-4 px-4 sm:px-6">
          ...
          <Button
            onClick={handleTestBrandCompose}
            disabled={brandTesting}
            variant="outline"
            className="w-full"
          >
            {brandTesting ? (
